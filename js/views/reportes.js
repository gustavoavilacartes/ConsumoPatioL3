...
export async function renderReportes(root) {
  ...
  async function draw() {
    ...
    table.appendChild(el('thead', {}, el('tr', {}, ['Folio', 'Tractor', 'Columna', 'Tipo', 'Línea', 'Vol. carga', 'Vol. descarga', 'Hora descarga'].map(h => el('th', {}, h)))));
    const tbody = el('tbody');
    let totalCarga = 0, totalDescarga = 0;
    viajes.forEach(v => {
      totalCarga += v.volumenCarga;
      totalDescarga += v.volumenDescarga || 0;
      tbody.appendChild(el('tr', {}, [v.folio, v.tractorNombre, v.columnaNombre, v.tipoMadera, v.lineaNombre, fmtM3(v.volumenCarga), fmtM3(v.volumenDescarga), fmtHora(v.horaDescarga)].map(t => el('td', {}, t))));
    });
